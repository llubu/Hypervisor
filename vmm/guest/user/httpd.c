...
send_data(struct http_request *req, int fd)
{
	...
	struct Stat st;
	struct Fd *fDesc = (struct Fd*) (0xD0000000 + (uint64_t)fd*PGSIZE);
	devfile.dev_stat(fDesc, &st); // this will never fail since every check has been done before

	char buf[st.st_size];
	int r;
	devfile.dev_read(fDesc, buf, st.st_size);
	if (write(req->sock, buf, st.st_size) != st.st_size)
		return -1;
	return 0;
...
send_file(struct http_request *req)
{
	...
	if((r = open(req->url, O_RDONLY)) < 0) {
		...
		fd = r;
		if((r = devfile.dev_stat((struct Fd*) (0xD0000000 + (uint64_t)r*PGSIZE), &st)) < 0) {
			send_error(req, 404);
			goto end;
		}
...
{
    cprintf("STARTING HTTPD SERVER IN GUEST \n");
